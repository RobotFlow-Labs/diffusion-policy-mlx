"""Diffusion UNet Hybrid Image Policy — the main policy class.

Upstream: diffusion_policy/policy/diffusion_unet_hybrid_image_policy.py

Ties together:
  - ``MultiImageObsEncoder``: encodes RGB + low-dim observations
  - ``ConditionalUnet1D``: denoises action trajectories
  - ``DDPMScheduler`` / ``DDIMScheduler``: noise schedule
  - ``LinearNormalizer``: data normalization

Two conditioning modes:
  - ``obs_as_global_cond=True`` (default): observations encoded as a global
    conditioning vector.  UNet input is ``(B, horizon, action_dim)``.
  - ``obs_as_global_cond=False``: observations concatenated with noisy actions
...
class DiffusionUnetHybridImagePolicy(BaseImagePolicy):
    """Vision-conditioned diffusion policy using a 1D UNet denoiser.
    ...
    def predict_action(self, obs_dict: Dict[str, mx.array]) -> Dict[str, mx.array]:
        ...
        if self.obs_as_global_cond:
            ...
            # Empty conditioning data for inpainting (all False mask)
    ...
    def compute_loss(self, batch: Dict) -> mx.array:
        ...
        # MSE loss with mask
        loss = (pred - target) ** 2
        # Apply loss mask (only compute loss on non-conditioned dims)
        loss = loss * loss_mask.astype(loss.dtype)
        # Mean over features, then over batch
        # Equivalent to: reduce(loss, 'b ... -> b (...)', 'mean').mean()
